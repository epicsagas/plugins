...
| [kanban-dev-lane](#kanban-dev-lane) | Автономная многомоторная полоса реализации — делегирует код в изолированные git-worktree с автоматическим фейловером (Claudy ➔ Codex ➔ AGYD). | [epicsagas/plugins/.hermes/kanban-dev-lane](https://github.com/epicsagas/plugins/tree/main/.hermes/kanban-dev-lane) |
...
**Автономная многомоторная полоса реализации для Hermes Kanban**

Делегирует ограниченные задачи реализации и рефакторинга из воркера Hermes Kanban в изолированный git-worktree с автоматической **3-уровневой цепочкой фейловера** (`Claudy` ➔ `Codex --yolo` ➔ `AGYD` ➔ `Hermes Direct`), обеспечивая непрерывный прогресс даже при исчерпании квот внешних провайдеров.

**Ключевые возможности:**
- Автоматическая детекция 429 и исчерпания квоты с фейловером без простоя
- Управление жизненным циклом изолированных git-worktree
- Hermes строго владеет состоянием Kanban, сверкой диффов и регрессионными тестами
- Встроенный CLI-раннер: `python3 .hermes/kanban-dev-lane/scripts/lane_runner.py`

→ [Исходники и документация](https://github.com/epicsagas/plugins/tree/main/.hermes/kanban-dev-lane)
